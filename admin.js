...
(function () {
  ...
  var SCHEMAS = {
    ...
    ggArtists: {
      ...
      fields: [
        ...
        { key: "photo", label: "대표 사진 (얼굴)", type: "image", filePrefix: "artist" },
        { key: "bio", label: "소개글 (빈 줄로 문단 구분)", type: "paragraphs" },
        { key: "works", label: "작품 사진 (여러 장 선택 가능)", type: "images", filePrefix: "work" }
      ],
      makeNew: function (id) {
        return { id: id, name: "", photo: "", bio: [], works: [] };
      },
    ...
    ggPublications: {
      ...
      makeNew: function (id) {
        return { id: id, category: "book", title: "", publisher: "", size: "", pages: "", year: "", price: "", cover: "", link: "" };
      }
    },
    ggNews: {
      label: "뉴스",
      ...
      fields: [
        ...
        { key: "relatedExhibition", label: "관련 전시 (전시 상세페이지에 노출)", type: "select", options: function () {
          var opts = [{ value: "", label: "선택 안함" }];
          (window.ggExhibitions || []).forEach(function (e) {
            opts.push({ value: e.id, label: e.titleKo });
          });
          return opts;
        } }
      ],
      makeNew: function (id) {
        ...
        return { id: id, category: "film", title: "", date: today, dateLabel: today, banner: "", link: "", body: [], relatedExhibition: "" };
      }
    },
    ggContact: {
      label: "연락처",
      ...
      fields: [
        ...
        { key: "directorPhoto", label: "디렉터 인사말 - 사진", type: "image", filePrefix: "director" },
        { key: "directorName", label: "디렉터 인사말 - 이름", type: "text" },
        { key: "directorGreetingKo", label: "디렉터 인사말 (한글)", type: "textarea" },
        { key: "directorGreetingEn", label: "디렉터 인사말 (영문)", type: "textarea" },
        { key: "signageImages", label: "갤러리 사이니지 안내 이미지 (여러 장, 순서대로 표시)", type: "images", filePrefix: "signage" }
      ]
    }
  };
  ...
  function applyFieldValues(item, schema, values) {
    schema.fields.forEach(function (field) {
      ...
      if (field.type === "images") {
        ...
        item.__exportNames = item.__exportNames || {};
        var namesArr = (item.__exportNames[field.key] || []).slice();
        var outArr = arr.map(function (entry, idx) {
          if (entry && entry.__upload) {
            var ext = extFromMime(entry.mimeType) || "jpg";
            var name = (field.filePrefix || "img") + "_" + (item.id || slug(Date.now())) + "_" +
              (idx + 1) + "_" + Date.now().toString(36).slice(-4) + "." + ext;
            namesArr[idx] = name;
            return entry.dataUrl;
          }
          namesArr[idx] = undefined;
          return entry;
        });
        item.__exportNames[field.key] = namesArr;
        setPath(item, field.key, outArr);
        return;
      }
  ...
  function buildFieldRow(field, value) {
    ...
    if (field.type === "textarea") {
      ...
      // field.options can be a static array or a function returning one —
      // the latter is used for options that depend on live data (e.g. the
      // current list of exhibitions), so it's re-evaluated every time this
      // field is drawn rather than being frozen at schema-definition time.
      ...
      // Multi-image field (e.g. an artist's list of work photos): shows
      // existing images as removable thumbnails, plus a file input that
      // accepts multiple new files at once. The current array of items
      // (existing path strings mixed with freshly-uploaded {__upload}
      // objects) lives in the closure below and is exposed via
      // input.__getItems() for the save handler to read.
      var listEl = document.createElement("div");
      listEl.className = "gg-field-images-list";
      ...
      function renderList() {
        ...
        items.forEach(function (it, idx) {
          ...
          thumbWrap.className = "gg-field-image-thumb";
          var thumbImg = document.createElement("img");
          thumbImg.src = it && it.__upload ? it.dataUrl : it;
          thumbWrap.appendChild(thumbImg);
          var rmBtn = document.createElement("button");
          rmBtn.type = "button";
          rmBtn.className = "gg-field-image-remove";
          rmBtn.setAttribute("aria-label", "제거");
          rmBtn.textContent = "×";
      ...
      input.addEventListener("change", function () {
        ...
        Promise.all(
          files.map(function (file) {
            return resizeImageFile(file).then(function (result) {
              items.push({ __upload: true, dataUrl: result.dataUrl, mimeType: result.mimeType });
              pendingCount--;
            });
          })
        )
    ...
    saveBtn.addEventListener("click", function () {
      ...
      form.querySelectorAll("[data-field-key]").forEach(function (input) {
        ...
        if (type === "paragraphs") {
          ...
          values[key] = input.__getItems ? input.__getItems() : [];
        } else {
  ...
  function buildLoginModal(onSuccess) {
    ...
    modal.innerHTML =
      "<h3>관리자 로그인</h3>" +
      '<input type="text" class="gg-field-input" id="gg-login-pw" placeholder="비밀번호" autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false">' +
  ...
  function prepareForExport(data, schema) {
    ...
    var rewritten = items.map(function (rawItem) {
      ...
      schema.fields.forEach(function (field) {
        ...
        if (field.type === "images") {
          var rawArr = getPath(rawItem, field.key) || [];
          var namesArr = (rawItem.__exportNames && rawItem.__exportNames[field.key]) || [];
          var outArr = rawArr.map(function (rawEntry, idx) {
            if (rawEntry && /^data:/.test(rawEntry)) {
              var entryName = namesArr[idx] ||
                ((field.filePrefix || "img") + "_" + (rawItem.id || slug(Date.now())) + "_" + (idx + 1) + "." + (extFromDataUrl(rawEntry) || "jpg"));
              imageFiles.push({ name: entryName, dataUrl: rawEntry });
              return "img/" + entryName;
            }
            return rawEntry;
          });
          setPath(item, field.key, outArr);
  ...
  function mountToggle() {
    ...
    // Login is now triggered by clicking the "gguggum art centre" text in
    // the footer itself, rather than a separate "관리자" button.
    var nameBtn = document.getElementById("gg-footer-name");
    if (nameBtn && !nameBtn.dataset.ggBound) {
      nameBtn.dataset.ggBound = "1";
    ...
    function renderBar() {
      if (isUnlocked()) {
        ...
        // Empty — .gg-admin-bar:empty collapses it, so the footer-name
        // text is the only visible login trigger when logged out.
        bar.innerHTML = "";
      }
    }
  }
