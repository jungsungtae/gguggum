...
(function () {
  ...
  function renderExhibitionDetail(elId) {
    ...
    el.innerHTML =
      ...
      "</div>" +
      relatedNewsHtml(item.id);
  }

  function relatedNewsHtml(exhibitionId) {
    var related = (window.ggNews || []).filter(function (n) {
      return n.relatedExhibition === exhibitionId;
    }).sort(function (a, b) {
      return a.date < b.date ? 1 : a.date > b.date ? -1 : 0;
    });
    if (!related.length) return "";
    return (
      '<div class="exhibition-related-news">' +
      "<h2>RELATED NEWS</h2>" +
      '<div class="news-grid">' + related.map(newsCardHtml).join("") + "</div>" +
      "</div>"
    );
  }
  ...
  function renderArtistDetail(elId) {
    ...
    el.innerHTML =
      ...
      "</div>" +
      artistWorksHtml(artist);
  }

  function artistWorksHtml(artist) {
    ...
    return (
      ...
      works.map(function (src) {
        return '<div class="artist-work-item"><img src="' + src + '" alt="' + esc(artist.name) + '"></div>';
      }).join("") +
      "</div>" +
      "</div>"
    );
  ...
  function renderPublications(elId, category) {
    var el = document.getElementById(elId);
    if (!el) return;
    var pubs = window.ggPublications || [];
    if (category) {
      pubs = pubs.filter(function (p) {
        return (p.category || "book") === category;
      });
    }
    el.innerHTML = pubs.length
      ? pubs.map(pubRowHtml).join("")
  ...
  function renderContact(ids) {
    ...
    if (ids.directorPhoto) {
      var elDirPhoto = document.getElementById(ids.directorPhoto);
      if (elDirPhoto) {
        if (c.directorPhoto) {
          elDirPhoto.setAttribute("src", c.directorPhoto);
          elDirPhoto.style.display = "";
        } else {
          elDirPhoto.style.display = "none";
        }
      }
    }
    if (ids.directorName) {
      var elDirName = document.getElementById(ids.directorName);
      if (elDirName) elDirName.textContent = c.directorName || "";
    }
    if (ids.directorKo) {
      var elDirKo = document.getElementById(ids.directorKo);
      if (elDirKo) elDirKo.innerHTML = c.directorGreetingKo ? nl2br(c.directorGreetingKo) : "디렉터 인사말을 준비 중입니다.";
    }
    if (ids.directorEn) {
      var elDirEn = document.getElementById(ids.directorEn);
      if (elDirEn) elDirEn.innerHTML = c.directorGreetingEn ? nl2br(c.directorGreetingEn) : "";
    }
    if (ids.signageImages) {
      var elSignage = document.getElementById(ids.signageImages);
      if (elSignage) {
        var images = c.signageImages || [];
        elSignage.innerHTML = images.map(function (src) {
          return '<img src="' + src + '" alt="Gallery signage guide">';
        }).join("");
      }
    }
    if (ids.root) {
